import XMLParser.WikiLinksReducer;
import XMLParser.WikiPageLinksMapper;
import XMLParser.XmlInputFormat;
import RankCalculate.RankCalculateMapper;
import RankCalculate.RankCalculateReduce;
import RankResult.RankingMapper;
import RankResult.RankingReducer;
...
public class WikiPageRanking extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        boolean isCompleted = runXmlParsing("/data/wiki/enwiki-latest-pages-articles.xml", "coursework2/PageRank/iter00");
        if (!isCompleted) return 1;

        String lastResultPath = null;

        for (int runs = 0; runs < 5; runs++) {
            String inPath = "coursework2/PageRank/iter" + nf.format(runs);
            lastResultPath = "coursework2/PageRank/iter" + nf.format(runs + 1);
        ...
        isCompleted = runRankOrdering(lastResultPath, "coursework2/PageRank/result");
	...
	rankOrdering.setNumReduceTasks(1);
        ...
        rankOrdering.setOutputFormatClass(MapFileOutputFormat.class);
